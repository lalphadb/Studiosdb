...
<div class="wizard-container">
    ...
    <form id="wizardForm" method="POST" action="{{ route('admin.membres.store') }}" enctype="multipart/form-data">
        <script>document.getElementById("wizardForm").addEventListener("submit", function(e) { console.log("Formulaire soumis!"); });</script>
        @csrf
        ...
        <div class="wizard-step-content" data-step="2" style="display: none;">
            <div class="glass-card max-w-4xl mx-auto">
                ...
                @else
                @if(auth()->user()->ecole_id @if(auth()->user()->hasEcole() && auth()->user()->ecole)@if(auth()->user()->hasEcole() && auth()->user()->ecole) auth()->user()->ecole)
